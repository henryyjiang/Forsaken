...
{
    public static SewerCamera I { get; private set; }
    ...
    public List<CameraRig> FirstFloorRigs = new();
    public List<CameraRig> SecondFloorRigs = new();
    ...
    public float PrevPlayerY { get; private set; }
    public float CurrPlayerY { get; private set; }
    private int floorThresholdIndex;
    ...
    private void Awake()
    {
        ...
        if (player) {
            PrevPlayerX = CurrPlayerX = player.position.x;
            PrevPlayerY = CurrPlayerY = player.position.y;
        }
    }
    ...
    private int FindFirstRigRightOfX(float x, float y)
    {
        bool onGroundFloor = y < floorThreshold;
        int startIndex = 0, endIndex = cameraRigs.Count;
        if (onGroundFloor)
        {
            endIndex = floorThresholdIndex;
        } else
        {
            startIndex = floorThresholdIndex;
        }
    ...
    private void FixedUpdate()
    {
        ...
        PrevPlayerY = CurrPlayerY;
        CurrPlayerY = player.position.y;

        ApplyRigSample(CurrPlayerX, CurrPlayerY);
    }
    ...
    public void RebuildCache()
    {
        ...
        //FirstFloorRigs.Sort((a, b) => a.x.CompareTo(b.x));
        found.AddRange(FirstFloorRigs);
        floorThresholdIndex = found.Count;
        //SecondFloorRigs.Sort((a, b) => a.x.CompareTo(b.x));
        found.AddRange(SecondFloorRigs);
    ...
    private void ApplyRigSample(float playerX, float playerY)
    {
        ...
        int right = FindFirstRigRightOfX(playerX, playerY);
